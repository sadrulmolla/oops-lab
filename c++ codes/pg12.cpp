...
public:
	void get(void)
	{
		...
		cout<<"Enter the Amount: ";
		cin>>blance;
		cout<<"Enter the type of the Account(1.Saving/2.Current)";
		cin>>typeacnt;
		time_t t = time(NULL);
		tm* timePtr = localtime(&t);
		day=(int)timePtr->tm_mday;
	...
	int return_typeacc(void){return typeacnt;}
	void display(void)
	{
		cout<<"\n===================================================\n";
		...
		cout<<"Account Type: ";
		if(typeacnt==1){cout<<"Saving Account"<<endl;}
		else {cout<<"Current Account"<<endl;}
		cout<<"\n====================================================\n";
	}
...
public:
	...
	void update(void)
	{
		...
		if(check_id(tran.return_accnum())>=0)
		{
			...
			int opt=list[n].return_typeacc();
			switch(opt)
			{
				case 1:
					if(list[n].return_blance()<tran.return_amount()+500 && tran.return_type()==1)
					{
					...
					break;
				case 2:
					if(list[n].return_blance()+20000<tran.return_amount()&& tran.return_type()==1)
					{
						cout<<"Transaction Faild.";
					}
					else
